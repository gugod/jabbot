...
## Contributed by kcwu
  ...
  my @urlmatch=(		[
		  ...
		  sub { "東森新聞報 - [ ".$title." ]： ".Lingua::ZH::Summarize::summarize($_[0],maxlength => 70 )." " }
		  #sub { "東森新聞報：[ ".$_[0]." ]" }
		],
		[
			'tw.news.yahoo.com',
		   sub { ($title) = ($_[0]=~m/<font face="標楷體" size="5" color="#0000cc">\n([^<]*)\n\s*<\/font>/is); $_[0] =~ m/<!--== article begin ==-->(.+)<!--== article end ==-->/is },
		   sub { $_[0] =~ s/<script.+<\/script>//mg ; $_[0]},
		   sub { $_[0] =~ s/<[^<]+>//mg ; $_[0]},
		   #sub { $_[0] =~ s/\n//mg; $_[0] },
		   sub { trim_whitespace(@_) ; @_ },
		   sub { "[ ".$title." ]： ".Lingua::ZH::Summarize::summarize($_[0],maxlength => 70 )." " }
		],
		  [
		  'http://',
